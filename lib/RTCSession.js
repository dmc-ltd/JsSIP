...
module.exports = class RTCSession extends EventEmitter {
  ...
  connect(target, options = {}, initCallback) {
    ...
    const eventHandlers = options.eventHandlers ? options.eventHandlers : {};
    const extraHeaders = Utils.cloneArray(options.extraHeaders);
    const mediaConstraints = options.mediaConstraints ? options.mediaConstraints : { audio: true, video: true };
    const mediaStream = options.mediaStream ? options.mediaStream : null;
    const pcConfig = options.pcConfig ? options.pcConfig : { iceServers: [] };
    const rtcConstraints = options.rtcConstraints ? options.rtcConstraints : null;
    const rtcOfferConstraints = options.rtcOfferConstraints ? options.rtcOfferConstraints : null;

    this._rtcOfferConstraints = rtcOfferConstraints;
    this._rtcAnswerConstraints = options.rtcAnswerConstraints ? options.rtcAnswerConstraints : null;
    ...
    const anonymous = options.anonymous ? options.anonymous : false;
  ...
  answer(options = {}) {
    ...
    const extraHeaders = Utils.cloneArray(options.extraHeaders);
    const mediaConstraints = options.mediaConstraints ? options.mediaConstraints : {};
    const mediaStream = options.mediaStream ? options.mediaStream : null;
    const pcConfig = options.pcConfig ? options.pcConfig : { iceServers: [] };
    const rtcConstraints = options.rtcConstraints ? options.rtcConstraints : null;
    const rtcAnswerConstraints = options.rtcAnswerConstraints ? options.rtcAnswerConstraints : null;
    ...
    this._rtcOfferConstraints = options.rtcOfferConstraints ? options.rtcOfferConstraints : null;
  ...
  terminate(options = {}) {
    ...
    const cause = options.cause ? options.cause : JsSIP_C.causes.BYE;
    const extraHeaders = Utils.cloneArray(options.extraHeaders);
    ...
    switch (this._status) {
      ...
      case C.STATUS_1XX_RECEIVED:
        ...
        if (status_code && (status_code < 200 || status_code >= 700)) {
          ...
          reason_phrase = reason_phrase ? reason_phrase : JsSIP_C.REASON_PHRASE[status_code] ? JsSIP_C.REASON_PHRASE[status_code] : '';
      ...
      case C.STATUS_ANSWERED:
        ...
        status_code = status_code ? status_code : 480;
      ...
      case C.STATUS_CONFIRMED:
        ...
        reason_phrase = options.reason_phrase ? options.reason_phrase : JsSIP_C.REASON_PHRASE[status_code] ? JsSIP_C.REASON_PHRASE[status_code] : '';
  ...
  sendDTMF(tones, options = {}) {
    ...
    let position = 0;
    let duration = options.duration ? options.duration : null;
    let interToneGap = options.interToneGap ? options.interToneGap : null;
  ...
  _createLocalDescription(type, constraints) {
    ...
    console.log('DMC', 'createLocalDescription()');

    if (type !== 'offer' && type !== 'answer') {
      console.log('DMC exception 1');
    ...
    console.log('DMC', 'start resolve()');

    return Promise.resolve()
      // Create Offer or Answer.
      .then(() => {
        console.log('DMC', 'inside 1st resolve()');
        if (type === 'offer') {
          console.log('DMC', 'type offer return create');
          return connection.createOffer(constraints)
            .catch((error) => {
              debugerror('emit "peerconnection:createofferfailed" [error:%o]', error);

              console.log('DMC', 'peerconnection:createofferfailed');
      ...
      .then((desc) => {
        console.log('DMC', 'set local description');
        if (type === 'answer') {
          console.log('DMC', 'set local desc answer type');
          if (connection.iceGatheringState === 'complete') {
            desc.sdp = desc.sdp.replace(/a=inactive/g, 'a=recvonly');
            desc.type = 'answer';
            console.log('DMC', 'state was complete');
          }
          else {
            desc.sdp = desc.sdp.replace(/a=recvonly/g, 'a=inactive');
            desc.type = 'pranswer';
            console.log('DMC', 'set type to pranswer');
          }
        }

        return connection.setLocalDescription(desc)
          .catch((error) => {
            console.log('DMC', 'error in local desc', error);
            this._rtcReady = true;
      ...
      .then(() => {
        ...
        console.log('DMC', 'above check for complete ice candidate', connection.iceGatheringState);
        if (connection.iceGatheringState === 'complete') {
          this._rtcReady = true;
          console.log('DMC', 'inside complete function');

          const e = { originator: 'local', type: type, sdp: connection.localDescription.sdp };
          console.log('DMC', 'e set to', e);

          debug('emit "sdp"');

          this.emit('sdp', e);
          console.log('DMC', 'emitted sdp');

          return Promise.resolve(e.sdp);
        }
        console.log('DMC', 'after ice candidate completed');
  ...
  _receiveReinvite(request) {
    ...
    function reject(options = {}) {
      ...
      const status_code = options.status_code ? options.status_code : 403;
      const reason_phrase = options.reason_phrase ? options.reason_phrase : '';
      const extraHeaders = Utils.cloneArray(options.extraHeaders);
  ...
  _processInDialogSdpOffer(request) {
    ...
    for (const m of sdp.media) {
      ...
      const direction = m.direction ? m.direction : sdp.direction ? sdp.direction : 'sendrecv';
  ...
  _sendReinvite(options = {}) {
    ...
    const extraHeaders = Utils.cloneArray(options.extraHeaders);
    const eventHandlers = options.eventHandlers ? options.eventHandlers : {};
    const rtcOfferConstraints = options.rtcOfferConstraints ? options.rtcOfferConstraints :
      this._rtcOfferConstraints ? this._rtcOfferConstraints : null;
  ...
  _sendUpdate(options = {}) {
    ...
    const rtcOfferConstraints = options.rtcOfferConstraints ? options.rtcOfferConstraints :
      this._rtcOfferConstraints ? this._rtcOfferConstraints : null;
    const sdpOffer = options.sdpOffer ? options.sdpOffer : false;
  ...
  _handleSessionTimersInIncomingRequest(request, responseExtraHeaders) {
    ...
    if (request.session_expires && request.session_expires >= JsSIP_C.MIN_SESSION_EXPIRES) {
      ...
      session_expires_refresher = request.session_expires_refresher ? request.session_expires_refresher : 'uas';
    }
    else {
  ...
  _handleSessionTimersInIncomingResponse(response) {
    ...
    if (response.session_expires &&
      ...
      session_expires_refresher = response.session_expires_refresher ? response.session_expires_refresher : 'uac';
    }
    else {
      this._sessionTimers.currentExpires = this._sessionTimers.defaultExpires;
